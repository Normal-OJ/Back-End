...
@post.route('/', methods=['POST','PUT','DELETE'])
...
def modify_post(user, course, title, content, targetThreadId):
    ...
    if request.method == 'POST':
        ...
        #add course post
        elif targetThreadId is None:
            r = add_post(course, user, content, title)
        else :
            return HTTPError('Request is fail,course or targetThreadId must be none',403)
