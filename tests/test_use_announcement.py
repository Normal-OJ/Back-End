...
class TestAnnouncement(BaseTester):
    ...
    def test_get_invalid_announcement_list(self, client_student, monkeypatch):
        ...
        assert rv.json()['message'] == 'Cannot Access a Announcement'
    ...
    def test_get_none_announcement_list(self, client_student, monkeypatch):
        ...
        assert rv.json()['message'] == 'Announcement Not Found'
    ...
    def test_add_invalid(self, client_student, monkeypatch):
        ...
        assert rv.json()['message'] == 'Failed to Create Announcement'
    ...
    def test_edit_without_perm(self, forge_client):
        ...
        assert rv.status_code == 403, rv.json()
        assert rv.json()['message'] == 'Failed to Update Announcement'
    ...
    def test_delete_without_perm(self, forge_client):
        ...
        id = rv.json()['data'][0]['annId']
        client = forge_client('student')
        rv = client.request("DELETE", '/ann', json={
            'annId': id,
        })
        assert rv.status_code == 403, rv.json()
        assert rv.json()['message'] == 'Failed to Delete Announcement'
    ...
    def test_delete(self, client_admin):
        ...
        rv = client_admin.request("DELETE", '/ann', json={'annId': id})
        assert rv.status_code == 200
