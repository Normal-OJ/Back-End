...
#
# Loop popping the FIFO tail: a job whose hash has evaporated is skipped; a job
# that is no longer its submission's current_job is destroyed on the spot (the
# dispatch-side half of INV4 — a superseded rejudge job never runs); the first
# live current job is leased (attempts+1, state=leased) and its id returned.
# Empty queue → nil.
CLAIM_PENDING = '''
...
while true do
  ...
  if redis.call('EXISTS', job_key) == 1 then
    ...
    if current == jb then
      local attempts = tonumber(redis.call('HGET', job_key, 'attempts')) + 1
...
# 0. Never creates or resurrects a key — HSET runs only on the confirmed-owner
...
# this script's job). Not leased / not expired / missing hash → 0.
RECLAIM_EXPIRED = '''
if redis.call('SISMEMBER', KEYS[1], ARGV[1]) == 0 then
  return 0
end
